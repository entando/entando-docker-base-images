...
= Steps for modifying and building the base images

Please note that this project is not being built on Jenkins X yet. We do have a build
setup on our legacy Jenkins, but it does not manage versions correctly. For this
reason we still have to build this project manually. This will change once we have
created a pipeline for these project in Jenkins X.

1. Clone this repository locally: `git clone git@github.com:entando/entando-docker-base-images.git`
2. Create a new branch for your pull request: `git checkout -b my-pr`
3. Make changes to the files in the images you want to modify
4. Commit your changes to the branch: `git commit -a -m "your message"`
5. Tag your changes with the new version number: `git tag v6.2.342`
6. Push your tagged changes, create a PR and wait for approval `git push origin my-pr --tags`
7. After PR approval and the merge, checkout master locally: `git checkout origin master`
8. Pull the latest changes `git pull origin master`
9. Log into Docker Hub with a user that has read/write access to the 'entando' organization
10. Build and push with the correct version number from the commandline: `./build-test-and-push.sh 6.2.342`

Tips:

* You can uncomment the 'docker push' command in the script build-test-and-push.sh to test your changes locally first
