...
if [ -n "$PORTDB_DRIVER" ]; then
  ...
  export PORTDB_SQL_CHECK=$($(dirname ${BASH_SOURCE[0]})/determine-sql-check-query.sh ${PORTDB_DRIVER})
fi
if [ -n "$SERVDB_DRIVER" ]; then
  export SERVDATASOURCECLASSNAME=$($(dirname ${BASH_SOURCE[0]})/determine-driver.sh ${SERVDB_DRIVER})
  export SERVDB_SQL_CHECK=$($(dirname ${BASH_SOURCE[0]})/determine-sql-check-query.sh ${$SERVDB_DRIVER})
fi
